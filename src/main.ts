...
import { encodeErrorPayload, ErrorCode } from "./errors/index.js";
import type { CounselError } from "./errors/index.js";
import { AgentPlatformService } from "./services/AgentPlatform.js";
import { RunService } from "./services/Run.js";

const normalizeVersionAlias = (args: ReadonlyArray<string>): ReadonlyArray<string> => {
...
const rawArgs = normalizeVersionAlias(process.argv.slice(2));
process.argv = [...process.argv.slice(0, 2), ...rawArgs];
const wantsJson = hasFlag(rawArgs, "--json");
...
const validateJsonArgs = (args: ReadonlyArray<string>): string | undefined => {
...
const jsonUsageError = wantsJson ? validateJsonArgs(rawArgs) : undefined;
...
const ServiceLayer = RunService.layer.pipe(
  Layer.provideMerge(AgentPlatformService.layer),
